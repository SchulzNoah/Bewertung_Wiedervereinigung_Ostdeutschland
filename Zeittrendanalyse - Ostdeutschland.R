# Zeittrendanalyse - Ost-Deutschland  --------
  ...
  allbus %>%
    mutate(eastwest = case_when(
      ...
      pr05 = case_when(
        ...
        pr05 == 3 ~ 2,
        pr05 == 4 ~ 1)) %>%
    filter(pr05 != -11, eastwest == "Neue Bundesländer") %>%
...
# Berechnung der Prozentanteile für jedes mögliche Jahr pro Kategorie
...
df_zeittrend %>%
  ...
  geom_area(position = "stack",
            ...
            color = "grey50", 
            linewidth = 0.2) +
  labs(title = "Zeittrendanalyse: Bewertung der Wiedervereinigung aus Sicht Ostdeutschlands",
       subtitle = "'Die Wiedervereinigung hat den neuen Bundesländern mehr Vor- als Nachteile gebracht' (Befragte: nur Ostdeutsche)",
       x = "Jahr",
       y = "Prozentualer Anteil",
       fill = "Bewertung") +
  scale_fill_manual(values = c("1" = "#a6bddb",  
                               "2" = "#3690c0",  
                               "3" = "#045a7f",  
                               "4" = "#023839"), 
