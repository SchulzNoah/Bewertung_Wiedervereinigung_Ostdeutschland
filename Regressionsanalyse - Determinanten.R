...
library(interactions) # Interaktionsplot
...
stargazer(m1, m2, m3, m4, m5, m6,
          type = "html",
          style = "ajps",
          title = "Regressionstabelle: Determinanten der Bewertung der Wiedervereinigung in Ostdeutschland",
...
# Verteilung der abhängigen Variablen (notwendig, da weniger als 5 Ausprägungen)
...
# Visualisierung der Ergebnisse der Breusch-Pagan-Tests
...
plot(m1, 1, main = "Homoskedastizitätsüberprüfung (m1)")
plot(m2, 1, main = "Homoskedastizitätsüberprüfung (m2)")
plot(m3, 1, main = "Homoskedastizitätsüberprüfung (m3)")
plot(m4, 1, main = "Homoskedastizitätsüberprüfung (m4)")
plot(m5, 1, main = "Homoskedastizitätsüberprüfung (m5)")
plot(m6, 1, main = "Homoskedastizitätsüberprüfung (m6)")
...
plot(m1, 2, main = "Normalverteilung der Residuen (m1)")
plot(m2, 2, main = "Normalverteilung der Residuen (m2)")
plot(m3, 2, main = "Normalverteilung der Residuen (m3)")
plot(m4, 2, main = "Normalverteilung der Residuen (m4)")
plot(m5, 2, main = "Normalverteilung der Residuen (m5)")
plot(m6, 2, main = "Normalverteilung der Residuen (m6)")
...
# Visualisierung der Multikollinearität pro Modell
...
check_collinearity(m1) %>%
  ...
  gt() %>%
  tab_header(title = "VIF-Werte m1 (Testung der Multikollinearität)") %>%
  gt_theme_espn()
...
check_collinearity(m2) %>%
  ...
  mutate(across(2:4, ~ round(., 2))) %>%  
  gt() %>%
  tab_header(title = "VIF-Werte m2 (Testung der Multikollinearität)") %>%
  gt_theme_espn()
...
check_collinearity(m3) %>%
  ...
  mutate(across(2:4, ~ round(., 2))) %>%
  gt() %>%
  tab_header(title = "VIF-Werte m3 (Testung der Multikollinearität)") %>%
  gt_theme_espn()
...
check_collinearity(m4) %>%
  ...
  mutate(across(2:4, ~ round(., 2))) %>%  
  gt() %>%
  tab_header(title = "VIF-Werte m4 (Testung der Multikollinearität)") %>%
  gt_theme_espn()
...
check_collinearity(m5) %>%
  ...
  mutate(across(2:4, ~ round(., 2))) %>%  
  gt() %>%
  tab_header(title = "VIF-Werte m5 (Testung der Multikollinearität)") %>%
  gt_theme_espn()
...
check_collinearity(m6) %>%
  ...
  mutate(across(2:4, ~ round(., 2))) %>%  
  gt() %>%
  tab_header(title = "VIF-Werte m6 (Testung der Multikollinearität)") %>%
  gt_theme_espn()
...
# Visualisierung der Ergebnisse der Durbin-Watson-Tests
...
plot(m1, 4, main = "Ausreißer-Diagnose (m1)")
plot(m2, 4, main = "Ausreißer-Diagnose (m2)")
plot(m3, 4, main = "Ausreißer-Diagnose (m3)")
plot(m4, 4, main = "Ausreißer-Diagnose (m4)")
plot(m5, 4, main = "Ausreißer-Diagnose (m5)")
plot(m6, 4, main = "Ausreißer-Diagnose (m6)")
...
# Berechnung der robusten Standardfehler
...
df_ost_stand = df_ost %>%
  select(`Bewertung der Wiedervereinigung in Ostdeutschland`, Alter, Geschlecht, `Wohnort (Stadt/Land)`,
         ...
         `Relative Deprivation: Gerechtigkeitsempfinden`) %>%
  mutate(across(-c(Geschlecht, `Wahl der Linkspartei`, `Arbeitslosigkeit in den letzten zehn Jahren`, `Wahl der AfD`), scale))
...
plot_coefs(m1_sr, m2_sr, m3_sr, m4_sr, m5_sr, m6_sr,
  ...
  labs(x = "Standardisierte Regressionskoeffizienten mit robusten Standardfehlern",
       y = "",
       title = "Effektstärke der Prädiktoren für die Bewertung der Wiedervereinigung aus ostdeutscher Perspektive") +
  scale_y_discrete(labels = c("Alter" = "Alter",
                              "Geschlecht" = "Geschlecht (Mann: 1, Frau: 2)",
                              "`Wohnort (Stadt/Land)`" = "Wohnort (Stadt/Land)",
                              "`Formeller Schulabschluss`" = "Formeller Schulabschluss",
                              ...
                              "`Arbeitslosigkeit in den letzten zehn Jahren`:`Wahl der AfD`" = 
                                "Arbeitslosigkeit in den letzten zehn Jahren \n* Wahl der AfD"))+
  scale_x_continuous(limits = c(-1.36, 1.0)) +
  theme(plot.title = element_text(hjust = .7,
        ...
        plot.margin = margin(t = 15, b = 10, unit = "pt"))



# Plot für die Interaktion zwischen Arbeitslosigkeit und AfD-Wahl ----------------------------------------------

interact_plot(m6, 
              pred = `Arbeitslosigkeit in den letzten zehn Jahren`, 
              modx = `Wahl der AfD`, 
              robust = T, 
              vcovHC = "HC3", 
              interval = T, 
              int.type = "confidence", 
              modx.labels = c("Nein", "Ja"),
              main.title = "Interaktion zwischen Arbeitslosigkeit und Wahl der AfD in Bezug 
              auf die Bewertung der Wiedervereinigung",
              x.label = "Arbeitslosigkeit in den letzten zehn Jahren", 
              y.label = "Bewertung der Wiedervereinigung") +
  theme_lucid(base_family = "serif")+
  labs(subtitle = "Regressionsmodell: m6")+
...
?interact_plot
# Auflistung aller verwendeten Packages -----------------------------------
